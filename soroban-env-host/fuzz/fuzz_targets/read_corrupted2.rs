...
use soroban_env_host::xdr::{Hash, ScContractInstance, ScErrorCode, ContractExecutable, ScVal};
use soroban_env_host::{Vm, Frame, MeteredOrdMap};
    ...
    loop {
        ...
        fuzz!( |input: InputData| {
            ...
            let _ = host.with_frame(frame.clone(), || {
                ...
                let void_val: Val = Val::VOID.into();
                for e in sc_vals.into_iter() {
                    // replace all invalid generated ScVals by VOID (deduplicate afterwards) 
                    let v: Val = host.map_err(<Val>::try_from_val(host, &e)).unwrap_or(void_val);
                    keys_vec.push(v)
                }
                keys_vec.dedup_by(|a, b| a.get_payload() == b.get_payload());
